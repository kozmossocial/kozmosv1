...
function createFallbackArtifact(plan) {
  ...
  const artifactContent = [
    ...
    "      </div>",
    "      <div class=\"controls\">",
    "        <label>Layer mode<select id=\"mode\"><option value=\"balanced\">Balanced</option><option value=\"surface-heavy\">Surface-heavy</option><option value=\"system-heavy\">System-heavy</option></select></label>",
    "        <label>Complexity<input id=\"intensity\" type=\"range\" min=\"1\" max=\"10\" value=\"6\" /></label>",
    "        <label>Outcome tag<input id=\"tag\" placeholder=\"release-v1\" /></label>",
    "      </div>",
    `      <div style=\"margin-top:10px; display:flex; gap:8px;\"><button id=\"run\">${escapeHtml(variant.action)}</button><button id=\"reset\">Reset</button></div>`,
    "    </section>",
    "    <section class=\"panel\">",
    ...
    "      const intensity = clamp(Number((intensityEl && intensityEl.value) || 6), 1, 10);",
    "      const tag = (tagEl && tagEl.value || '').trim() || 'untagged';",
    "      const signal = clamp(Math.floor(a.length / 30), 0, 35);",
    "      const constraint = clamp(Math.floor(b.length / 30), 0, 35);",
    "      const coherence = clamp(49 + signal + Math.floor(constraint * 0.6) + Math.floor(intensity * 2.2), 0, 100);",
    "      const risk = clamp(90 - Math.floor((signal + constraint) * 0.6), 5, 95);",
    "      const expansionPressure = clamp(25 + Math.floor(intensity * 5.8), 20, 88);",
    "      const payload = {",
    ...
    "        risk_index: risk,",
    "        expansion_pressure: expansionPressure,",
    "        layers: {",
    "          surface: mode === 'system-heavy' ? 0.25 : 0.4,",
    "          system: mode === 'surface-heavy' ? 0.28 : 0.44,",
    "          expansion: mode === 'balanced' ? 0.3 : 0.32",
    "        },",
    ...
    "      aEl.value = '';",
    "      bEl.value = '';",
    "      if (modeEl) modeEl.value = 'balanced';",
    "      if (intensityEl) intensityEl.value = '6';",
    "      if (tagEl) tagEl.value = '';",
    "      outEl.textContent = 'Run to generate output...';",
    "    });",
    "    restore();",
    "  </script>",
    "</body>",
...
async function runSessionBuildMission({
  ...
  for (let attempt = 1; attempt <= missionMaxBundleAttempts; attempt += 1) {
    const bundlePrompt = [
      ...
      "- artifactContent must include at least six interactive controls/events total (inputs/buttons/select/range/canvas/details/listeners)",
      ...
      "- docs must explicitly describe three layers: surface layer, system layer, and expansion pathway",
      "- artifact must wire at least one Kozmos runtime integration point (KozmosRuntime kv/proxy/starter hooks when relevant)",
    ...
    if (countInteractiveElements(candidateBundle.artifactContent) < 6) continue;
