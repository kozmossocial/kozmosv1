...
function injectPreviewRuntimeBridge(
  ...
  const bridgeScript = [
    ...
    "        },",
    "        request: async function (toUsername) {",
    "          const token = window.KozmosRuntime.starter._getToken();",
    "          return req('/api/build/runtime/starter/friends', { method: 'POST', headers: token ? { 'x-kozmos-starter-token': token } : {}, body: JSON.stringify({ spaceId: __SPACE_ID__, toUsername: String(toUsername || '') }) });",
    "        },",
    "        respond: async function (requestId, action) {",
    "          const token = window.KozmosRuntime.starter._getToken();",
    "          return req('/api/build/runtime/starter/friends', { method: 'PATCH', headers: token ? { 'x-kozmos-starter-token': token } : {}, body: JSON.stringify({ spaceId: __SPACE_ID__, requestId: Number(requestId || 0), action: String(action || '') }) });",
    "        },",
    "        unfriend: async function (friendUserId) {",
    "          const token = window.KozmosRuntime.starter._getToken();",
    "          return req('/api/build/runtime/starter/friends', { method: 'DELETE', headers: token ? { 'x-kozmos-starter-token': token } : {}, body: JSON.stringify({ spaceId: __SPACE_ID__, friendUserId: String(friendUserId || '') }) });",
    "        }",
    "      }",
    "    }",
    "  };",
