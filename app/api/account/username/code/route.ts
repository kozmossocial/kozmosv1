...
const USERNAME_MAIL_FROM = process.env.KOZMOS_EMAIL_FROM || "axy@kozmos.social";
const USERNAME_CODE_TTL_MINUTES = 15;
...
async function sendUsernameChangeEmail(email: string, code: string) {
  const html = `
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111">
      <h2>Kozmos Username Change Verification</h2>
      <p>Your verification code is:</p>
      <p style="font-size:24px;letter-spacing:4px;"><strong>${code}</strong></p>
      <p>This code expires in ${USERNAME_CODE_TTL_MINUTES} minutes.</p>
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: USERNAME_MAIL_FROM,
      to: [email],
      subject: "Kozmos username change verification code",
      html,
    }),
  });
  if (!res.ok) {
    const msg = await res.text().catch(() => "");
    throw new Error(`email send failed (from=${USERNAME_MAIL_FROM}): ${msg || res.status}`);
  }
}
  ...
  try {
    ...
    if (!RESEND_API_KEY) {
      return NextResponse.json(
        { error: `email service not configured (from=${USERNAME_MAIL_FROM})` },
        { status: 500 }
      );
    }
    ...
    const expiresAt = new Date(Date.now() + USERNAME_CODE_TTL_MINUTES * 60_000).toISOString();

    const { error: insertErr } = await supabaseAdmin.from("user_username_change_codes").insert({
      user_id: user.id,
      code_hash: codeHash,
    ...
    await sendUsernameChangeEmail(user.email, code);
