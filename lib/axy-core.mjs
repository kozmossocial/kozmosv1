...
export function scoreMissionBundleQuality(bundle, options = {}) {
  ...
  const apiContract = String(bundle?.apiContract || "").trim();
  const exportManifest = String(bundle?.exportManifest || "").trim();
  ...
  const minImplementation = Number.isFinite(options.minImplementation)
    ...
    : 480;
  const minApiContract = Number.isFinite(options.minApiContract)
    ? Number(options.minApiContract)
    : 260;
  const minExportManifest = Number.isFinite(options.minExportManifest)
    ? Number(options.minExportManifest)
    : 120;
  const minArtifact = Number.isFinite(options.minArtifact) ? Number(options.minArtifact) : 260;
  const minInteractiveElements = Number.isFinite(options.minInteractiveElements)
    ? Number(options.minInteractiveElements)
    : 6;
  ...
  if (apiContract.length < minApiContract) {
    score -= 12;
    issues.push("api-contract-too-short");
  }
  if (!/^#\s+/m.test(apiContract)) {
    score -= 8;
    issues.push("api-contract-heading-missing");
  }
  if (exportManifest.length < minExportManifest) {
    score -= 10;
    issues.push("export-manifest-too-short");
  }
  try {
    const parsedExport = JSON.parse(exportManifest);
    if (!parsedExport || typeof parsedExport !== "object") {
      score -= 8;
      issues.push("export-manifest-invalid");
    }
  } catch {
    score -= 8;
    issues.push("export-manifest-invalid");
  ...
  if (!/(surface\s+layer|surface\s+functionality|interaction\s+layer)/i.test(docsCorpus)) {
    score -= 10;
    issues.push("surface-layer-missing");
  }
  if (!/(system\s+layer|orchestration|state\s+machine|policy\s+engine|decision\s+engine)/i.test(docsCorpus)) {
    score -= 10;
    issues.push("system-layer-missing");
  }
  if (!/(expansion\s+pathway|platform\s+expansion|phase\s*2|extension\s+path)/i.test(docsCorpus)) {
    score -= 10;
    issues.push("expansion-layer-missing");
  }

  const runtimeSignalCount = (
    artifactContent.match(
      /window\.KozmosRuntime|KozmosRuntime\.|kvSet\(|kvGet\(|starter\.|proxy\(|fetch\(/gi
    ) || []
  ).length;
  if (runtimeSignalCount < 2) {
    score -= 10;
    issues.push("runtime-integration-weak");
  }
  ...
  if (interactiveElements < minInteractiveElements) {
    score -= 14;
    issues.push("interaction-depth-low");
  }

  if (usageSteps.length < 3) {
    score -= 6;
    issues.push("usage-steps-too-few");
  }
  ...
  const ok = qualityScore >= 84 && issues.length === 0;
  return {
    ok,
    qualityScore,
    issues,
